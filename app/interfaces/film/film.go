...
func (r *filmRepository) Insert(newFilm model.FilmNew) (model.ID, *model.Error) {
	return r.conn.Upsert(newFilm)
}
func (r *filmRepository) Update(film model.Film) *model.Error {
	_, err := r.conn.Upsert(film)
	return err
}
...
func (r *filmRepository) GetByPipeline(pipeline interface{}) ([]model.Film, *model.Error) {
	filmsInterfaces, err := r.conn.GetByQuery(configs.Default.FilmTargetName, pipeline)
	if err != nil {
		return nil, err
	}
