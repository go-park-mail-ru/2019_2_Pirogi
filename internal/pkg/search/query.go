...
func (qp *QuerySearchParams) GetSearchResultsByQuery(conn database.Database) []interface{} {
	qp.filter()
	entries := conn.GetByQuery(configs.Default.FilmsCollectionName, qp.GetPipelineForMongo(configs.Default.FilmTargetName))
	if entries == nil {
		entries = conn.GetByQuery(configs.Default.PersonsCollectionName, qp.GetPipelineForMongo(configs.Default.PersonTargetName))
	}
	return entries
}
